...
if args.column_name:
    ...
    column_name = input('Enter column to match: ')

df_1 = pd.read_csv(filename, header=0)
df_2 = pd.read_csv(filename2, header=0)

df_1[column_name].str.strip()
df_2[column_name].str.strip()

df_1.sort_values(by=[column_name], inplace=True)
df_2.sort_values(by=[column_name], inplace=True)
...
frame = fuzzy_merge(df_1, df_2, column_name, column_name)
dt = datetime.now().strftime('%Y-%m-%d %H.%M.%S')
frame.to_csv('fuzzyMatchFor'+column_name+'_'+dt+'.csv', index=False)
